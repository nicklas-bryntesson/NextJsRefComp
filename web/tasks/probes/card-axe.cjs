/* card-axe.cjs — axe over /primitives/card in BOTH appearances.
 *
 * Derived from button-axe.cjs, which was itself derived from axe-dark.cjs.
 * Same reasoning: the Razor primitive set has no conformance suite, so this
 * probe and card-reflow.cjs are the whole accessibility net for Card.
 *
 * Card.css has no state rules and no transitions at all, so the transition kill
 * and the two-frame settle below are belt-and-braces rather than load-bearing
 * here — kept identical to the Button probe so the two are comparable.
 *
...
/* BASE_URL is parameterised (the Button probe hardcodes :3200) because several
 * primitive ports are running in parallel against one .next directory, and a
 * restart on the shared port pulls the ground out from under another agent's
 * measurement. Default kept at :3200 so it behaves like its sibling. */
...
(async () => {
  ...
  for (const appearance of ['light', 'dark']) {
    ...
    await page.goto(BASE + '/primitives/card');
