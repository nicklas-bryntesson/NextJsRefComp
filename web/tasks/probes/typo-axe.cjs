/* typo-axe.cjs — axe over the typographic pair's routes in BOTH appearances.
 *
 * Adapted from button-axe.cjs (itself adapted from axe-dark.cjs); its header
 * follows. Routes come from argv, defaulting to /primitives/{heading,prose}.
 *
 * The `exclude` list is EMPTY here. button-axe.cjs excludes
 * `[data-disabled="true"]` for the 1.4.3 inactive-component exception; neither
 * Heading nor Prose has a disabled state, so nothing on these routes qualifies
 * for that exemption and nothing is excluded. Notably that means the two
 * `opacity: 0.7` declarations Prose inherited verbatim were audited without
 * mercy — which is how they were caught.
 *
...
(async () => {
  ...
  const routes = process.argv.slice(2).length ? process.argv.slice(2)
    : ['/primitives/heading', '/primitives/prose'];
  for (const route of routes) {
  for (const appearance of ['light', 'dark']) {
    const page = await browser.newPage();
    await page.goto((process.env.BASE_URL || 'http://localhost:3210') + route);
    ...
    console.log(`\n=== ${route}  ${appearance.toUpperCase()} — ${v.length} violation type(s) ===`);
    for (const x of v) {
